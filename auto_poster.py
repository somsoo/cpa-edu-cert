...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["유치원 행정실무사 자격증 (적중 예상문제 제공)", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 산림치유 및 에코투어 전문 평생교육 교수입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "forest nature tree",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'forest nature tree')
